...
for i in range(1,6):
    ...
    list_recommender = items_tr.intersection(items_te)
    list_recommender = list(list_recommender)
    result = pd.Series(list_recommender)
    result.to_csv("D:\\TCC_2\\files-to-recommender\\items_rec_" + str(i) + ".result",sep="\t",encoding='utf8')
